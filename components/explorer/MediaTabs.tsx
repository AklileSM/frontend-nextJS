...
export function MediaTabs({ active, counts, onChange, railId = 'media-tab-rail' }: Props) {
  return (
    <div className="flex justify-center">
      <div
        role="tablist"
        className="inline-flex items-center gap-1 rounded-2xl border border-base-800 bg-base-800/50 p-1.5"
      >
        {TABS.map((t) => {
          ...
          return (
            <button
              ...
              className={`relative flex items-center gap-2.5 rounded-xl px-5 py-2.5 text-[15px] font-medium transition-colors ${
            ...
            >
              {isActive && (
                <motion.span
                  layoutId={railId}
                  className="absolute inset-0 rounded-xl bg-base-950 shadow-sm shadow-black/40"
                />
              )}
              <span className="relative">{t.label}</span>
              <span
                className={`relative rounded-md px-2 py-0.5 font-mono text-[11px] ${
